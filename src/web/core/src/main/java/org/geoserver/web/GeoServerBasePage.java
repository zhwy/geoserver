...
public class GeoServerBasePage extends WebPage implements IAjaxIndicatorAware {
	...
	/**
	 * The id of the panel sitting in the page-header, right below the page
	 * description
	 */
	...
	/**
	 * page class for this page to return to when the page is finished, could be
	 * null.
	 */
	...
	public GeoServerBasePage() {
		...
		// login / logout stuff
		List<String> securityFilters = getGeoServerApplication().getSecurityManager().getSecurityConfig()
				.getFilterChain().filtersFor("/web/**");
		...
		add(new ListView<LoginFormInfo>("loginforms", loginforms) {
			public void populateItem(ListItem<LoginFormInfo> item) {
				...
				List<GeoServerSecurityProvider> securityProviders = getGeoServerApplication()
				...
				for (GeoServerSecurityProvider securityProvider : securityProviders) {
					if (securityProvider.getFilterClass() != null
							&& securityProvider.getFilterClass().equals(info.getFilterClass())) {
						filterInChain = true;
						break;
					}
				}
		...
		add(new ListView<LogoutFormInfo>("logoutforms", logoutforms) {
			public void populateItem(ListItem<LogoutFormInfo> item) {
				...
				if (info.getIcon() != null) {
					...
					image = new Image("link.icon",
							new PackageResourceReference(GeoServerBasePage.class, "img/icons/silk/door-out.png"));
				}
		...
		add(new ListView<Category>("category", categories) {
			public void populateItem(ListItem<Category> item) {
				...
				item.add(new Label("category.header",
				...
				item.add(new ListView<MenuPageInfo>("category.links", links.get(category)) {
					public void populateItem(ListItem<MenuPageInfo> item) {
						...
						BookmarkablePageLink<Page> link = new BookmarkablePageLink<Page>("link",
	...
	public void renderHead(IHeaderResponse response) {
		...
		// includes jquery, required by the placeholder plugin (wicket only include
		// jquery if he
		// need it)
	...
	/**
	 * Gets the page title from the PageName.title resource, falling back on
	 * "GeoServer" if not found
	 */
	String getPageTitle() {
		try {
	...
	/**
	 * The base page is built with an empty panel in the page-header section that
	 * can be filled by subclasses calling this method
	 *
	 * @param component The component to be placed at the bottom of the page-header
	 *                  section. The component must have "page-header" id
	 */
	...
	/**
	 * Splits up the pages by category, turning the list into a map keyed by
	 * category
	 *
	 * @param pages
	 */
	...
	/**
	 * Filters a set of component descriptors based on the current authenticated
	 * user.
	 */
	...
	/**
	 * Returns the id for the component used as a veil for the whole page while
	 * Wicket is processing an ajax request, so it is impossible to trigger the same
	 * ajax action multiple times (think of saving/deleting a resource, etc)
	 *
	...
	/**
	 * Returns from the page by navigating to one of {@link #returnPage} or
	 * {@link #returnPageClass}, processed in that order.
	 *
	 * <p>
	 * This method should be called by pages that must return after doing some task
	 * on a form submit such as a save or a cancel. If no return page has been set
	 * via {@link #setReturnPage(Page)} or {@link #setReturnPageClass(Class)} then
	 * {@link GeoServerHomePage} is used.
	 */
	 ...
	 *
	 * <p>
	 * This method accepts a parameter to use as a default in cases where
	 * {@link #returnPage} and {@link #returnPageClass} are not set and a default
	 * other than {@link GeoServerHomePage} should be used.
	 *
	 * <p>
	 * This method should be called by pages that must return after doing some task
	 * on a form submit such as a save or a cancel. If no return page has been set
